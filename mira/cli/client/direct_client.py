...
class DirectClient:
    ...
    def _get_moorcheh(self):
        ...
        if self._moorcheh is None:
            from mira.app.clients.moorcheh import get_moorcheh_client
    ...
    def _get_read_service(self):
        ...
        if self._read_service is None:
            from mira.app.services.memory_read_service import MemoryReadService
    ...
    def _get_daily_analysis_service(self):
        ...
        if self._daily_analysis_service is None:
            from mira.app.services.daily_analysis_service import (
    ...
    def _get_export_service(self):
        ...
        if self._export_service is None:
            from mira.app.services.memory_export_service import MemoryExportService
    ...
    def _get_validated_session_for_agent(self, agent_id: str):
        ...
        # writes the new token to ~/.mira/sessions/{agent}.json and
    ...
    def create_agent(
        ...
        from mira.app.models.session import AgentCreate, AgentPattern
    ...
    def generate_conflict_report(self, agent_id: str, date: str) -> dict[str, Any]:
        ...
        memories and writes the JSON report to ``~/.mira/conflicts/``.
    ...
    def list_conflicts(
        ...
        json_path = (
            Path.home() / ".mira" / "conflicts" / f"{agent_id}_{date}_conflicts.json"
        )
    ...
    def resolve_conflict(
        ...
        from mira.app.core import agent_namespace
    ...
    def export_memory_md(
        ...
        Args:
            ...
            output_path: Custom output path. Defaults to
                ``~/.mira/exports/{agent_id}_memory.md``.
        ...
        from mira.app.services.memory_export_service import MEMORY_TYPE_ORDER
    ...
    def sync_memory_to_project(
        ...
        Uses the cached export at ``~/.mira/exports/{agent_id}_memory.md``
        ...
        cache_path = Path.home() / ".mira" / "exports" / f"{agent_id}_memory.md"
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        exists only for interface compatibility with ``MiraAPIClient``.
        ...
        raise ConnectionError(
            ...
            "Run 'mira serve' to start the server if needed."
        )
