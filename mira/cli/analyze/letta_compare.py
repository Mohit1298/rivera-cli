"""
Compare a Letta archival memory export against Mira.
...
def compute_metrics(export: dict[str, Any]) -> dict[str, Any]:
    ...
    storage_saved_bytes = storage_letta_bytes - storage_mira_bytes

    read_ms_letta = ASSUMPTIONS["letta_read_ms"]
    read_ms_mira = ASSUMPTIONS["mira_read_ms"]
    latency_speedup = (
        round(read_ms_letta / read_ms_mira, 1) if read_ms_mira else 0
    )
    ...
    return {
        ...
        "latency": {
            ...
            "ms_saved_per_query": read_ms_letta - read_ms_mira,
        },
    }
...
def build_llm_prompt(metrics: dict[str, Any]) -> str:
    ...
    return (
        ...
        "compares a user's existing Letta deployment against Mira "
        ...
        f"({s['letta_human']} across {v['vector_count']} vectors). Mira "
        ...
        "Neighbor (ANN) search over archival passages; Mira would use "
...
def build_report_markdown(
    *,
    ...
    lines: list[str] = []
    lines.append("# Mira vs. Letta — Memory Analysis Report")
    lines.append("")
    ...
    lines.append("| --- | --- | --- |")
    lines.append(
        f"| Read latency | ~{lat['letta_read_ms']}ms | <{lat['mira_read_ms']}ms |"
    )
    ...
    lines.append("| | Letta | Mira |")
    lines.append("| --- | --- | --- |")
    lines.append(f"| Vector storage | {s['letta_human']} | {s['mira_human']} |")
    lines.append("")
    lines.append(
