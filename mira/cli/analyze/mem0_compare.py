"""
Compare a Mem0 account export against Mira.
...
Two layers:
  ...
  2. A narrative written by Mira's own LLM (Rivera ``answer`` endpoint),
...
def compute_metrics(export: dict[str, Any]) -> dict[str, Any]:
    ...
    storage_mira_bytes = vector_count * ASSUMPTIONS["vector_bytes_mira"]
    storage_saved_bytes = storage_mem0_bytes - storage_mira_bytes

    read_ms_mem0 = ASSUMPTIONS["mem0_read_ms"]
    read_ms_mira = ASSUMPTIONS["mira_read_ms"]
    latency_speedup = round(read_ms_mem0 / read_ms_mira, 1) if read_ms_mira else 0
...
def build_llm_prompt(metrics: dict[str, Any]) -> str:
    ...
    return (
        ...
        "compares a user's existing Mem0 deployment against Mira "
        ...
        "=== PROJECTED MIRA IMPACT (if you migrate) ===\n"
        ...
        f"${t['mem0_extraction_cost_usd']} total. With Mira, typed "
        f"primitives could bypass that step → 0 tokens, $0.\n"
        f"2. Latency — Mem0 is ~{lat['mem0_read_ms']}ms read with indexing "
        f"before new memories are searchable. Mira could deliver "
        f"<{lat['mira_read_ms']}ms read and 0ms write (instantly searchable) — "
        f"about {lat['speedup_x']}x faster per query.\n"
        f"3. Storage — Mem0 currently stores Float32 vectors "
        f"({s['mem0_human']} across {v['vector_count']} vectors). Mira "
        f"could use {s['compression_ratio']}x binary compression ({s['mira_human']}), "
        ...
        "Neighbor (ANN) search; Mira would use deterministic exact-match recall "
        ...
        "- Future or conditional tense for Mira benefits (can save, would save, "
        "could improve, if you migrate). Do NOT write as if they already use Mira.\n\n"
...
def build_report_markdown(
    *,
    ...
    lines: list[str] = []
    lines.append("# Mira vs. Mem0 — Memory Analysis Report")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Projected impact of migrating to Mira")
    lines.append("")
    lines.append("### 1. Ingestion tax (token savings)")
    lines.append("")
    lines.append("| | Mem0 | Mira |")
    lines.append("| --- | --- | --- |")
    lines.append(
    ...
    lines.append(
        ...
        f"(input + output) at ingest if you migrate — Mira's typed primitives "
    ...
    lines.append("| --- | --- | --- |")
    lines.append(
        f"| Read latency | ~{lat['mem0_read_ms']}ms | <{lat['mira_read_ms']}ms |"
    )
    ...
    lines.append("| | Mem0 | Mira |")
    lines.append("| --- | --- | --- |")
    lines.append(f"| Vector storage | {s['mem0_human']} | {s['mira_human']} |")
    lines.append("")
    lines.append(
