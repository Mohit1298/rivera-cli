"""
Compare a Supermemory account export against Mira.
...
ASSUMPTIONS: dict[str, Any] = {
    ...
    # Mira/Rivera stores a 32x-compressed binary code per vector.
...
def compute_metrics(export: dict[str, Any]) -> dict[str, Any]:
    ...
    storage_mem_bytes = vector_count * ASSUMPTIONS["vector_bytes_mira"]
    storage_saved_bytes = storage_sm_bytes - storage_mem_bytes

    read_ms_sm = ASSUMPTIONS["supermemory_read_ms"]
    read_ms_mem = ASSUMPTIONS["mira_read_ms"]
...
def build_llm_prompt(metrics: dict[str, Any]) -> str:
    ...
    return (
        ...
        "compares a user's existing Supermemory deployment against Mira "
        ...
        f"${t['supermemory_extraction_cost_usd']} total. With Mira, typed "
        f"primitives could bypass that step → 0 tokens, $0.\n"
        f"2. Latency — Supermemory is ~{lat['supermemory_read_ms']}ms read with an "
        f"indexing delay before new memories are searchable. Mira could deliver "
        ...
        f"({s['supermemory_human']} across {v['vector_count']} vectors). Mira "
...
def build_report_markdown(
    *,
    ...
    lines: list[str] = []
    lines.append("# Mira vs. Supermemory — Memory Analysis Report")
    lines.append("")
    ...
    lines.append("| --- | --- | --- |")
    lines.append(
        f"| Vector storage | {s['supermemory_human']} | {s['mira_human']} |"
    )
    lines.append("")
    lines.append(
