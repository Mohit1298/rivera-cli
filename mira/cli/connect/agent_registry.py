...
class AgentDef:
    ...
    sentinel: str = "<!-- MIRA-MANAGED-SECTION -->"
    sentinel_end: str = "<!-- /MIRA-MANAGED-SECTION -->"
    ...
    def resolve_skill_local(self, project_dir: Path) -> Path:
        ...
        if self.skill_local_dir:
            return project_dir / self.skill_local_dir / "mira"
        return project_dir / ".agents" / "skills" / "mira"

    def resolve_skill_global(self) -> Path:
        """Resolve global skill directory path."""
        if self.skill_global_dir:
            return Path.home() / self.skill_global_dir.lstrip("~/") / "mira"
        return Path.home() / ".agents" / "skills" / "mira"
...
CLAUDE_CODE = AgentDef(
    ...
    hook_config=AgentHookConfig(
        ...
        hook_payload={
            ...
            "hooks": [
                {
                    "type": "command",
                    "command": "mira memory sync --project-dir .",
                    "timeout": 30,
                }
            ],
        },
    ...
    permissions_payload={"permissions": {"allow": ["Bash(mira:*)"]}},
)
...
CURSOR = AgentDef(
    name="cursor",
    display_name="Cursor",
    instruction_file=".cursor/rules/mira.mdc",
...
CLINE = AgentDef(
    name="cline",
    display_name="Cline",
    instruction_file=".clinerules/mira.md",
    instruction_format="markdown",
...
CONTINUE = AgentDef(
    ...
    instruction_file=".continue/rules/mira.md",
    instruction_format="markdown",
    instruction_is_dir=True,
...
ROO = AgentDef(
    ...
    instruction_file=".roo/rules/mira.md",
    instruction_format="markdown",
    instruction_is_dir=True,
...
AUGMENT = AgentDef(
    ...
    instruction_file=".augment/rules/mira.md",
    instruction_format="markdown",
    instruction_is_dir=True,
...
def detect_mira_installed(project_dir: Path) -> list[AgentDef]:
    """Detect which agents have MIRA already installed (local)."""
    installed = []
    for agent in AGENT_REGISTRY.values():
...
def detect_mira_installed_global() -> list[AgentDef]:
    """Detect which agents have MIRA installed globally."""
    installed = []
    for agent in AGENT_REGISTRY.values():
