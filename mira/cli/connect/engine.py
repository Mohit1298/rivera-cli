"""
MIRA CLI - Connect Engine

Core logic for installing/removing MIRA integration to AI coding agents.
...
from mira.cli.connect.agent_registry import AGENT_REGISTRY, AgentDef
from mira.cli.connect.templates import (
    MIRA_SENTINEL,
    MIRA_SENTINEL_END,
...
def install_agent(
    ...
    """Install MIRA integration for a single agent.
...
def remove_agent(
    ...
    """Remove MIRA integration for a single agent."""
    agent = AGENT_REGISTRY.get(agent_name)
    if not agent:
        return {
            "agent": agent_name,
...
def _install_instructions(
    ...
    """Install MIRA instructions into the agent's instruction file."""
...
def _write_dedicated_file(file_path: Path, content: str) -> str:
    ...
    if file_path.exists():
        ...
        if MIRA_SENTINEL in existing:
            # Replace existing section
            pattern = (
                re.escape(MIRA_SENTINEL) + r".*?" + re.escape(MIRA_SENTINEL_END)
            )
...
def _inject_into_file(
    ...
    """Inject MIRA section into an existing file, or create it."""
    if file_path.exists():
        existing = file_path.read_text(encoding="utf-8")
        if MIRA_SENTINEL in existing:
            # Replace existing section
            ...
            file_path.write_text(updated, encoding="utf-8")
            return f"Updated MIRA section in {file_path.name}"
        else:
            ...
            file_path.write_text(updated, encoding="utf-8")
            return f"Added MIRA section to {file_path.name}"
...
def _remove_instructions(
    ...
    """Remove MIRA instructions from the agent's instruction file."""
    if not agent.instruction_file:
        return None
    ...
    if MIRA_SENTINEL in existing:
        pattern = re.escape(MIRA_SENTINEL) + r".*?" + re.escape(MIRA_SENTINEL_END)
        updated = re.sub(pattern, "", existing, flags=re.DOTALL)
        # Clean up extra whitespace
        updated = re.sub(r"\n{3,}", "\n\n", updated).strip() + "\n"
        if updated.strip():
            instr_path.write_text(updated, encoding="utf-8")
            return f"Removed MIRA section from {instr_path.name}"
        else:
            instr_path.unlink()
...
def _install_hooks(agent: AgentDef, project_path: Path, is_global: bool) -> str | None:
    ...
    # Check if mira hook already exists
    mira_exists = any(
        isinstance(group, dict)
        and any(
            isinstance(h, dict) and "mira" in h.get("command", "")
