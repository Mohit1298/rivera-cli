"""
MIRA CLI - Agent commands (create, list, activate, deactivate, delete, bootstrap).
"""
...
def agent_create(
    ...
    try:
        ...
        console.print(
            '[dim]You can now run: mira remember "..." and mira recall "..."[/dim]'
        )
...
def agent_list():
    ...
    try:
        ...
        if not agents:
            console.print(
                "[yellow]No agents found. Create one with 'mira agent create'[/yellow]"
            )
            return

        table = Table(
            title="MIRA Agents", show_header=True, header_style=BOLD_PRIMARY
        )
...
def agent_activate(
    ...
    try:
        ...
        if "not found" in msg.lower():
            hint = "Run 'mira agent list' to see available agents, or 'mira agent create <id>' to create one."
...
def agent_delete(
    ...
    try:
        ...
        hint = None
        if "not found" in msg.lower():
            hint = "Run 'mira agent list' to see available agents."
        _error(f"Failed to delete agent '{agent_id}': {msg}", hint=hint)

    if not keep_cloud:
        namespace = f"mira_agent_{agent_id}"
...
def agent_bootstrap(
    ...
    Examples:
        mira agent bootstrap
        mira agent bootstrap my-agent
        mira agent bootstrap --output snapshot.json
    """
    ...
    if total_sampled == 0:
        ...
        console.print(
            Panel(
                ...
                '[dim]Store memories with: mira remember "some fact"[/dim]',
