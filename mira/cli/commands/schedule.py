"""
MIRA CLI - Schedule commands (enable, disable, status).
"""
...
from mira.cli.schedule_manager import ScheduleManager
...
def schedule_run_internal(
    ...
    run ``mira daily-summary`` or ``mira detect-conflicts`` instead.
    """
    ...
    if config_manager.get_backend() == Backend.ON_PREM:
        _error(
            ...
            hint="Switch with: mira config backend cloud",
        )
    ...
    if not agent_id:
        if not active_agent_id:
            _error(
                "No active agent.",
                hint="Run 'mira agent activate <agent-id>' first.",
            )
    ...
    if result.get("enabled"):
        ...
        console.print(
            Panel(
                ...
                "[dim]Run 'mira schedule enable' to activate the nightly "
