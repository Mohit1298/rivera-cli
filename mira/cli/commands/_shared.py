"""
MIRA CLI - Shared utilities, app instances, and helpers.
...
from mira.app.utils.errors import InvalidSessionTokenError, SessionExpiredError

# Re-export temporal helpers
from mira.app.utils.temporal_helpers import (  # noqa: F401
...
from mira.cli.connect.agent_registry import (  # noqa: F401
    AGENT_REGISTRY,
...
from mira.cli.connect.engine import install_agent, remove_agent  # noqa: F401

# Re-export display functions
from mira.cli.ui.display import print_logo, show_welcome_banner  # noqa: F401

# Re-export theme constants for all command modules
from mira.cli.ui.theme import (  # noqa: F401
    ACCENT,
    BOLD_BRIGHT,
    BOLD_PRIMARY,
    BRIGHT,
...
app = typer.Typer(
    name="mira",
    help="MIRA CLI - Memory that AI Agents Love!",
...
connect_app = typer.Typer(help="Connect MIRA to external tools")
migrate_app = typer.Typer(
    help="Migrate memories from other providers (Mem0/Letta/Supermemory) into Mira"
)
...
def get_client() -> SdkClient:
    ...
    if backend == Backend.ON_PREM:
        ...
        if not cfg_key:
            _error(
                "MIRA not configured.",
                hint="Run 'mira' to set up your API key.",
            )
        api_key = cfg_key
