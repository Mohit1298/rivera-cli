...
Outputs live in ``~/.mira/migrate/<provider>/<timestamp>/`` to keep the
...
from mira.cli.migrate.runner import (
    load_export,
    run_migration,
    write_preview,
)
...
def _resolve_provider_key(
    ...
    if not entered or not entered.strip():
        _error(
            ...
            hint=f"Pass --api-key or set {env_name} in ~/.mira/.env",
        )
    set_fn(entered.strip())
    console.print("[green]  ✓ API key saved to ~/.mira/.env[/green]")
    resolved = get_fn()
    if not resolved:
...
def _generate_narrative(prompt: str, *, provider_label: str) -> tuple[str, str, str]:
    ...
    if not active_agent_id:
        _warn(
            ...
            "Run 'mira agent activate <agent-id>' to include it."
        )
        ...
        try:
            ...
            result = client.answer(
                ...
                header_prompt=(
                    ...
                    "Mira benefits. Output clean markdown. Do not fabricate "
                    "benchmark numbers."
                ),
            ...
            last_error = str(exc)
            if attempt == 0:
                _warn("Mira session invalid — re-activating agent and retrying...")
                try:
...
def _resolve_target_agent(agent: str | None) -> str:
    ...
    if not active_agent_id or not active_session_token:
        _error(
            ...
            hint=(
                "Activate an agent first ('mira agent activate <id>') "
                "or pass --agent <id>."
            ),
        )
...
def _run_migrate_flow(
    *,
    ...
    console.print(
        Panel.fit(
            f"[{BOLD_PRIMARY}]{label} -> Mira  {mode}[/{BOLD_PRIMARY}]",
            border_style=PRIMARY,
        )
    )
    ...
    progress("Mapping source records onto Mira schema...")
...
def migrate_mem0(
    api_key: str | None = typer.Option(
        ...
        "--api-key",
        envvar="MEM0_API_KEY",
        help="Mem0 API key (saved to ~/.mira/.env)",
    ),
    ...
    agent: str | None = typer.Option(
        ...
        "--agent",
        "-a",
        help="Target Mira agent id (defaults to the active agent).",
    ),
    ...
    """Migrate a Mem0 account into the active (or selected) Mira agent.

    Examples:
        mira migrate mem0 --dry-run
        mira migrate mem0 --file ./mem0_export.json
        mira migrate mem0 --agent my-agent --report
    """
    _run_migrate_flow(
        provider="mem0",
        api_key=api_key,
...
def migrate_letta(
    api_key: str | None = typer.Option(
        ...
        help="Letta API key (saved to ~/.mira/.env)",
    ),
    file: Path | None = typer.Option(
        None,
        "--file",
    ...
    """Migrate Letta archival passages into the active (or selected) Mira agent."""
    _run_migrate_flow(
        provider="letta",
        api_key=api_key,
        file=file,
...
def migrate_supermemory(
    api_key: str | None = typer.Option(
        ...
        help="Supermemory API key (saved to ~/.mira/.env)",
    ),
    file: Path | None = typer.Option(
        None,
        "--file",
    ...
    """Migrate a Supermemory account into the active (or selected) Mira agent."""
    _run_migrate_flow(
