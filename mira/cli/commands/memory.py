"""
MIRA CLI - Memory commands (remember, recall, answer, daily-summary,
...
def remember(
    ...
    Single memory:  mira remember "some fact"
    Batch mode:     mira remember --batch memories.json
    """
    ...
    if not active_agent_id or not active_session_token:
        _error(
            "No active agent.", hint="Run 'mira agent activate <agent-id>' first."
        )
    ...
    if not content:
        _error(
            ...
            "Try 'mira remember --help' for help.",
        )
        ...
        with console.status("[cyan]Searching memories...", spinner="dots"):
            if as_of:
                ...
                _error(
                    ...
                    hint="Try 'mira recall --help' for help.",
                )
        elapsed = time.perf_counter() - start
...
def daily_summary(
    ...
    if not agent_id:
        if not active_agent_id:
            _error(
                "No active agent.",
                hint="Provide an agent ID or run 'mira agent activate <agent-id>' first.",
            )
        agent_id = active_agent_id
    ...
    try:
        ...
        console.print(
            ...
            "Run 'mira detect-conflicts' or enable the schedule "
            "with 'mira schedule enable'.[/dim]"
        )
...
def detect_conflicts(
    ...
    and writes the JSON report to ``~/.mira/conflicts/``. Resolve the
    detected conflicts interactively with ``mira conflicts``.

    This is the command the schedule runs — see ``mira schedule``.
    """
    start = time.perf_counter()
    active_agent_id, _ = config_manager.get_active_session()
    ...
    try:
        ...
        if conflicts.get("status") == "success":
            ...
            if count > 0:
                ...
                console.print(
                    "[dim]  Run 'mira conflicts' to resolve interactively[/dim]"
                )
            else:
...
def conflicts(
    ...
    Examples:
        mira conflicts
        mira conflicts --date 2026-03-01
        mira conflicts --list
        mira conflicts --agent my-agent
    """
    active_agent_id, active_session_token = config_manager.get_active_session()
    ...
    if not unresolved:
        ...
        console.print(
            "[dim]Run 'mira detect-conflicts' to generate a conflict report.[/dim]"
        )
        return
    ...
    if not active_session_token:
        _error(
            ...
            "Run 'mira agent activate <agent-id>' first.",
        )
