"""
MIRA CLI - Memory management commands (export, sync).
"""
...
from mira.app.services.memory_export_service import (
    MEMORY_TYPE_META,
    MEMORY_TYPE_ORDER,
)
...
def memory_export(
    ...
    Examples:
        mira memory export
        mira memory export --agent my-agent
        mira memory export -o ./memory.md
        mira memory export -n 50
    """
    ...
    if not agent_id:
        if not active_agent_id or not active_session_token:
            _error(
                ...
                hint="Provide --agent or run 'mira agent activate <agent-id>' first.",
            )
...
def memory_sync(
    ...
    Examples:
        mira memory sync
        mira memory sync --project-dir ./my-project
        mira memory sync -p C:\\\\Projects\\\\my-app --agent my-agent
    """
    start = time.perf_counter()
    active_agent_id, active_session_token = config_manager.get_active_session()
