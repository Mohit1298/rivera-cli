"""
MIRA CLI - Config commands (show, backend).
"""
...
def config_show():
    ...
    table = Table(title="MIRA Configuration", show_header=False)
...
def config_backend(
    ...
    if target not in {"cloud", "on-prem"}:
        _error(
            ...
            hint="Use: mira config backend cloud  OR  mira config backend on-prem",
        )
    ...
    if target_backend == Backend.CLOUD:
        if not config_manager.get_api_key():
            from mira.cli.commands.core import _cloud_setup

            _cloud_setup()
    else:
        from mira.cli.commands.core import _onprem_setup
    ...
    try:
        ...
        _settings.MIRA_BACKEND = target_backend.value
