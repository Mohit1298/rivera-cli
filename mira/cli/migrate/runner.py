...
Pipeline:
    ...
    2. Map source rows → Mira memory payloads via ``mappers.MAPPERS``.
...
from mira.cli.migrate.mappers import MAPPERS, type_breakdown
...
def write_preview(rows: list[dict[str, Any]], dest: Path) -> Path:
    """Write the mapped Mira payloads so a dry-run is fully inspectable."""
