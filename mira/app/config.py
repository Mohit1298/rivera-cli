...
# Load project .env first, then ~/.mira/.env for the API key
load_dotenv()
_mira_env = Path.home() / ".mira" / ".env"
if _mira_env.exists():
    load_dotenv(_mira_env, override=True)

# Load model override from ~/.mira/config.yaml
_config_file = Path.home() / ".mira" / "config.yaml"
if _config_file.exists():
    try:
        ...
        with open(_config_file) as f:
            ...
            _mira = _data.get("mira", {})

            # Answer configuration
            _answer = _mira.get("answer", {})
            ...
            _backend = _mira.get("backend")
            if _backend:
                os.environ["MIRA_BACKEND"] = str(_backend)
    except Exception:
        pass

    # On-prem URL lives in ~/.mira/on-prem/state.json so on-prem onboarding
    # never has to touch the shared cloud yaml.
    try:
        import json as _json

        _state_path = Path.home() / ".mira" / "on-prem" / "state.json"
...
class Settings(BaseSettings):
    ...
    MIRA_SECRET_KEY: str = "mira-default-secret-change-in-production"
...
def get_data_dir() -> Path:
    ...
    isolated under ``~/.mira/on-prem/``.
    """
    base = Path.home() / ".mira"
    if settings.MIRA_BACKEND.strip().lower() == "on-prem":
        d = base / "on-prem"
