...
from mira.app.routes.auth_deps import get_current_session, get_session_service
...
from mira.app.utils.errors import AuthorizationError, map_error_to_http_exception
from mira.app.utils.validation import CostGuard
...
async def remember(
    ...
    try:
        ...
        from mira.app.constants import MemoryType, ProvenanceType
...
async def batch_remember(
    ...
    Accepts up to 100 memories per request. Leverages Rivera's batch
...
async def upload_file(
    ...
    The file is processed by Rivera to extract text and generate embeddings,
...
async def answer(
    ...
    Uses Rivera's answer.generate endpoint to produce LLM-generated answers
    ...
    try:
        ...
        # Use Rivera's answer.generate endpoint. Threshold is required
...
async def generate_daily_summary(
    ...
    try:
        result = await asyncio.to_thread(
            DirectClient(settings.RIVERA_API_KEY).generate_daily_summary,
            agent_id,
            resolved_date,
            request.output_path,
        )
...
async def generate_conflict_report(
    ...
    try:
        result = await asyncio.to_thread(
            DirectClient(settings.RIVERA_API_KEY).generate_conflict_report,
            agent_id,
            resolved_date,
        )
        return {
...
async def list_conflicts(
    ...
    try:
        conflicts = await asyncio.to_thread(
            DirectClient(settings.RIVERA_API_KEY).list_conflicts,
            agent_id,
            date,
        )
        return {
...
async def resolve_conflict(
    ...
    try:
        result = await asyncio.to_thread(
            DirectClient(settings.RIVERA_API_KEY).resolve_conflict,
            agent_id,
            resolved_date,
