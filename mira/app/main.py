...
from mira.app.routes import health, sessions
from mira.app.ui.routes.ui_router import mount_ui_static
from mira.app.ui.routes.ui_router import router as ui_router
...
def _validate_startup_dependencies() -> None:
    ...
    if not api_key:
        raise RuntimeError(
            "RIVERA_API_KEY is not configured. Set it before starting MIRA."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "RIVERA_API_KEY is invalid. Update it and restart MIRA."
        ) from exc
    except Exception as exc:
...
# Create FastAPI app
app = FastAPI(
    title="Mira - Memory that AI Agents Love!",
