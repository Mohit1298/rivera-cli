...
from mira.cli.config.manager import ConfigManager
from mira.cli.connect.agent_registry import AGENT_REGISTRY, list_agents
from mira.cli.connect.engine import install_agent, remove_agent

router = APIRouter()

# Shared ConfigManager instance (reads from ~/.mira/)
...
def _build_ui_direct_client() -> DirectClient | None:
    ...
    require a real Mira API key; returns ``None`` when one isn't
...
async def get_ui_config():
    """
    Get current MIRA configuration for the Web UI.
...
async def update_ui_config(updates: dict):
    """
    Update non-sensitive MIRA configuration from the Web UI.
...
def _update_onprem_answer(ans: dict) -> None:
    ...
    """
    from mira.cli.commands.core import _recover_moorcheh_api_key
    ...
    if not embedding_provider or not embedding_model:
        raise HTTPException(
            status_code=400,
            detail="On-prem not onboarded — run `mira config backend on-prem` first.",
        )
...
async def list_conflict_scans(agent_id: str | None = None):
    ...
    conflicts_dir = Path.home() / ".mira" / "conflicts"
    scans: dict[str, dict] = {}
...
async def read_daily_summary(agent_id: str | None = None, date: str | None = None):
    ...
    from mira.app.config import get_data_dir
...
async def get_connections():
    ...
    been installed (per registry at `~/.mira/connections.json`).
    """
        ...
        items.append(
            {
                ...
                "skill_local_template": (
                    f"{agent.skill_local_dir}/mira"
                    if agent.skill_local_dir
                    else ".agents/skills/mira"
                ),
                "skill_global_path": (
                    f"{agent.skill_global_dir}/mira"
...
async def connections_install(body: dict):
    """Install MIRA integration for one or more agents at a given location.
...
async def connections_uninstall(body: dict):
    """Remove MIRA integration for a single agent at a given location.
...
async def shutdown_server(background_tasks: BackgroundTasks):
    """
    Gracefully shutdown the MIRA server.
    Called by the UI when the browser tab is closed.
    """
    if not settings.MIRA_UI_MODE:
...
def _migrate_compact_metrics(provider: str, metrics: dict) -> dict:
    ...
    return {
        ...
        "read_ms_mira": latency.get("mira_read_ms", 0),
...
def _migrate_load_or_export(
    ...
    """
    from mira.cli.analyze.letta_export import run_letta_export
    from mira.cli.analyze.mem0_export import run_mem0_export
    from mira.cli.analyze.supermemory_export import run_supermemory_export
    from mira.cli.migrate.runner import load_export
...
def _migrate_get_metrics_fn(provider: str):
    if provider == "mem0":
        from mira.cli.analyze.mem0_compare import compute_metrics

        return compute_metrics
    if provider == "letta":
        from mira.cli.analyze.letta_compare import compute_metrics

        return compute_metrics
    from mira.cli.analyze.supermemory_compare import compute_metrics
...
async def migrate_dry_run(body: dict):
    ...
    """
    from mira.cli.migrate.runner import map_export, source_count
...
async def migrate_import(body: dict):
    ...
    """
    from mira.cli.migrate.runner import run_migration
    ...
    client = _build_ui_direct_client()
    if client is None:
        raise HTTPException(status_code=400, detail="No Mira API key configured.")
