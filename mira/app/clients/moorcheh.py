...
on-prem client (``mira.app.clients.onprem.OnPremClient``), based on
``settings.MIRA_BACKEND``.
...
class MoorchehClientSingleton:
    ...
    def _backend(self) -> Backend:
        return parse_backend(settings.MIRA_BACKEND)
    ...
    def get_client(self, api_key: str | None = None) -> Any:
        ...
        if self._backend() == Backend.ON_PREM:
            if self._client is None:
                from mira.app.clients.onprem import OnPremClient
    ...
    def get_async_client(self, api_key: str | None = None) -> Any:
        ...
        if self._backend() == Backend.ON_PREM:
            if self._async_client is None:
                from mira.app.clients.onprem import AsyncOnPremClient
