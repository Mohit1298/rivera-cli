...
codebase imports from `mira/app/legacy/`, and this folder is **excluded from
...
the `mira conflicts` CLI, not by supersession flags. All of the following were
deleted:
...
**`mira/app/services/memory_read_service.py`**
...
|---|---|---|
| `mira/app/utils/idempotency.py` | `legacy/idempotency.py` | `IdempotencyHandler` / `handle_write_idempotency` never imported anywhere |
| `mira/app/utils/tracing.py` | `legacy/tracing.py` | trace span/decorator helpers never imported |
| `mira/app/utils/safe_deletion.py` | `legacy/safe_deletion.py` | `SafeDeletion.perform_safe_deletion` never imported |
| `mira/app/models/phase_d.py` | `legacy/phase_d.py` | Phase-D models never imported |
| `mira/app/models/universal_endpoints.py` | `legacy/universal_endpoints_models.py` | referenced only by the already-dead legacy `universal_*` files (renamed to avoid colliding with the existing `legacy/universal_endpoints.py` routes file) |
...
`mira_agent_{agent_id}`. The broader multi-scope abstraction
...
- `MemoryReadService.search_multi_scope()`: removed. Called only from the dead
  `legacy/memory.py`, and it had a broken `from mira.mira.app.constants`
  import (double `mira`) proving it never ran.
...
its namespace is `mira_agent_{agent_id}` (unchanged on the wire).
...
- `mypy mira` → no issues (legacy excluded)
- `import mira.app.main` / `import mira.cli.main` → OK
