"""
Task definitions for the CrewAI + Mira integration example.
"""
...
def create_research_task(agent: Agent, topic: str = "AI agent frameworks") -> Task:
    ...
    return Task(
        description=(
            ...
            "For each key finding, use the mira_remember tool to store it with:\n"
            ...
            "After storing all findings, use mira_recall to verify they were saved "
            "by searching for them."
        ),
        expected_output=(
            "A confirmation that all research findings have been stored in Mira, "
...
def create_writing_task(agent: Agent, topic: str = "AI agent frameworks") -> Task:
    ...
    return Task(
        description=(
            ...
            "Steps:\n"
            "1. Use mira_recall with query 'market research findings' to retrieve "
            "all stored research (set limit to 10)\n"
            "2. Use mira_recall with specific queries like 'market size', "
            "'key players', 'growth trends' to find targeted information\n"
            "3. Use mira_answer to synthesize insights: "
