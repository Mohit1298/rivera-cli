...
|----------|-----------------|-------------|
| `POST /api/v2/agents` | Server `RIVERA_API_KEY` | Create a new agent namespace |
| `DELETE /api/v2/agents/{id}?delete-backup-too={true|false}` | Server `RIVERA_API_KEY` | Delete local agent; `true` also deletes Moorcheh namespace backup |
| `POST /api/v2/agents/{id}/activate` | Server `RIVERA_API_KEY` | Start session, get token |
| `POST /api/v2/agents/{id}/deactivate` | Server `RIVERA_API_KEY` | End the current agent session |
