...
from rivera.app.config import get_data_dir, settings
...
from rivera.app.utils.ids import generate_id
from rivera.app.utils.temporal_helpers import utc_now
...
def get_session_service() -> "SessionService":
    ...
    if _session_service is None:
        _session_service = SessionService(secret_key=settings.RIVERA_SECRET_KEY)
...
class SessionService:
    ...
    def __init__(self, secret_key: str | None = None, sessions_dir: Path | None = None):
        ...
        Args:
            ...
            sessions_dir: Directory for session storage (defaults to ~/.rivera/sessions/)
        """
        resolved_secret_key = (
            secret_key
            or os.getenv("RIVERA_SECRET_KEY")
            or "rivera-default-secret-change-in-production"
        )
    ...
    def log_memory_to_session_summary(
        ...
        Args:
            ...
            memory_id: The Rivera memory ID (if available)
        """
    ...
    def log_memory_deletion_to_session_summary(
        ...
        Args:
            agent_id: The agent's identifier
            session_id: The current session's identifier
            memory_id: The Rivera memory ID that was deleted
        """
        dt_now = utc_now()
        timestamp = dt_now.strftime("%Y-%m-%d %H:%M:%S")
        date_str = dt_now.strftime("%Y-%m-%d")
