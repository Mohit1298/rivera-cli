...
from rivera.app.utils.ids import generate_memory_id


class MemoryWriteService:
    """Persist memory records to Rivera-backed namespaces."""

    def __init__(self, moorcheh_client: "MoorchehClient"):
        """Initialize the service with a Rivera client."""
    ...
    def batch_store_memories(
        ...
        """
        Store multiple memories in batch leveraging Rivera's 100 docs/request capability
        ...
        try:
            ...
            if len(memories) > 100:
                raise MemoryError(
                    f"Batch size {len(memories)} exceeds Rivera's limit of 100 documents per request"
                )
            ...
            # Upload all validated documents in single batch to Rivera
    ...
    def update_memory(
        ...
        Since Rivera doesn't support in-place updates, we:
        ...
        try:
            ...
            # The namespace (rivera_agent_{agent_id}) is authoritative for the
            ...
            if not agent_id and namespace.startswith("rivera_agent_"):
                agent_id = namespace.removeprefix("rivera_agent_")
            if not agent_id:
                raise MemoryError(
