...
from rivera.app.clients.backend import get_active_llm_model
from rivera.app.config import settings
from rivera.app.core import agent_namespace
from rivera.app.utils.errors import MemoryError
...
class MemoryReadService:
    ...
    def namespace_service(self):
        if self._namespace_service is None:
            from rivera.app.services.namespace_service import NamespaceService
    ...
    def search_memories(
        ...
        """
        Search memories with filters leveraging Rivera's native metadata filtering

        Uses Rivera's #key:value syntax for efficient server-side filtering
        ...
        try:
            ...
            # Build enhanced query with Rivera metadata filters
            ...
            # Request extra results to handle offset (Rivera doesn't have native offset support)
            requested_limit = limit + offset
            top_k = min(requested_limit, 100)  # Rivera max is 100
            ...
            # Apply temporal filtering (post-processing since Rivera metadata filters are string-based)
    ...
    def _fetch_all_memories(
        ...
        """
        List all stored memories across the given namespaces via Rivera's
    ...
    def _build_filtered_query(
        ...
        """
        Build enhanced query with Rivera's #key:value metadata filters
        ...
        since Rivera's metadata filters use string comparison
        """
        filter_parts = []
    ...
    def _filter_expired_memories(
        ...
        This provides application-level TTL enforcement since Rivera doesn't
    ...
    def _filter_search_results(
        ...
        Note: This is a fallback filter. Primary filtering should use Rivera's
    ...
    def _format_memory_item(self, item: Any) -> dict[str, Any]:
        ...
        # Check if metadata is in nested format (Rivera API spec)
        ...
        # Parse title and content from Rivera document text format:
