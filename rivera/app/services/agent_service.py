...
from rivera.app.models.session import AgentCreate, AgentInfo, AgentList
...
class AgentService:
    ...
    def __init__(self, agents_dir: Path | None = None):
        ...
        Args:
            agents_dir: Directory for agent metadata storage (defaults to ~/.rivera/agents/)
        """
    ...
    def _generate_namespace(self, agent_id: str) -> str:
        """
        Generate the Rivera namespace for an agent.
    ...
    def create_agent(
        ...
        Args:
            ...
            moorcheh_api_key: Rivera API key for namespace creation
        ...
        # Create namespace in Rivera - CRITICAL: Must succeed.
        # ``moorcheh_api_key`` is honored on cloud; ignored on on-prem.
        client = get_moorcheh_client()

        try:
            # Use Rivera SDK to create namespace with type="text"
            client.namespaces.create(namespace, type="text")
            print(f"[OK] Namespace created in Rivera: {namespace}")
        except ConflictError:
            # Namespace already exists - this is OK, agent might have been created before
            print(f"[OK] Namespace already exists in Rivera: {namespace}")
            ...
            if ("namespace" in msg and "already exists" in msg) or "conflict" in msg:
                ...
                raise Exception(
                    f"Failed to create namespace '{namespace}' in Rivera: {str(e)}"
                )
