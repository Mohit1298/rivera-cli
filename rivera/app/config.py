...
# Load project .env first, then ~/.rivera/.env for the API key
load_dotenv()
_rivera_env = Path.home() / ".rivera" / ".env"
if _rivera_env.exists():
    load_dotenv(_rivera_env, override=True)

# Load model override from ~/.rivera/config.yaml
_config_file = Path.home() / ".rivera" / "config.yaml"
if _config_file.exists():
    try:
        ...
        with open(_config_file) as f:
            ...
            _rivera = _data.get("rivera", {})

            # Answer configuration
            _answer = _rivera.get("answer", {})
            ...
            _backend = _rivera.get("backend")
            if _backend:
                os.environ["RIVERA_BACKEND"] = str(_backend)
    except Exception:
        pass

    # On-prem URL lives in ~/.rivera/on-prem/state.json so on-prem onboarding
    # never has to touch the shared cloud yaml.
    try:
        import json as _json

        _state_path = Path.home() / ".rivera" / "on-prem" / "state.json"
        if _state_path.exists():
            _state = _json.loads(_state_path.read_text())
            _op_url = _state.get("url")
            if _op_url:
                os.environ["RIVERA_ONPREM_URL"] = str(_op_url)
            _op_embed = _state.get("embedding_provider")
            if _op_embed:
                os.environ["RIVERA_ONPREM_EMBEDDING_PROVIDER"] = str(_op_embed)
    except Exception:
        pass
...
class Settings(BaseSettings):
    ...
    # Rivera Configuration
    RIVERA_API_KEY: str = ""
    RIVERA_BASE_URL: str = "https://api.wirtel.ca"
    ...
    RIVERA_SECRET_KEY: str = "rivera-default-secret-change-in-production"
    ...
    RIVERA_SCHEDULE_TIME: str = "23:55"

    # Auto Parsing Configuration
    AUTO_PARSE_ENABLED: bool = True

    # UI Mode
    RIVERA_UI_MODE: bool = False
...
def get_data_dir() -> Path:
    ...
    isolated under ``~/.rivera/on-prem/``.
    """
    base = Path.home() / ".rivera"
    if settings.RIVERA_BACKEND.strip().lower() == "on-prem":
        d = base / "on-prem"
