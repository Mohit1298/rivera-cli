...
from rivera.cli.connect.agent_registry import AGENT_REGISTRY, list_agents
from rivera.cli.connect.engine import install_agent, remove_agent

router = APIRouter()

# Shared ConfigManager instance (reads from ~/.rivera/)
...
def _build_ui_direct_client() -> DirectClient | None:
    ...
    require a real Rivera API key; returns ``None`` when one isn't
...
async def get_ui_config():
    """
    Get current RIVERA configuration for the Web UI.
    ...
    return {
        ...
        "ui_mode": settings.RIVERA_UI_MODE,
    }
...
async def update_ui_config(updates: dict):
    """
    Update non-sensitive RIVERA configuration from the Web UI.
...
def _update_onprem_answer(ans: dict) -> None:
    ...
    """
    from rivera.cli.commands.core import _recover_moorcheh_api_key
    ...
    if not embedding_provider or not embedding_model:
        raise HTTPException(
            status_code=400,
            detail="On-prem not onboarded — run `rivera config backend on-prem` first.",
        )
...
async def update_api_key(body: dict):
    """
    Update the Rivera API key from the Web UI.
...
async def list_conflict_scans(agent_id: str | None = None):
    ...
    conflicts_dir = Path.home() / ".rivera" / "conflicts"
    scans: dict[str, dict] = {}
...
async def read_daily_summary(agent_id: str | None = None, date: str | None = None):
    ...
    from rivera.app.config import get_data_dir
...
async def get_connections():
    ...
    been installed (per registry at `~/.rivera/connections.json`).
    """
        ...
        items.append(
            {
                ...
                "skill_local_template": (
                    f"{agent.skill_local_dir}/rivera"
                    if agent.skill_local_dir
                    else ".agents/skills/rivera"
                ),
                "skill_global_path": (
                    f"{agent.skill_global_dir}/rivera"
...
async def connections_install(body: dict):
    """Install RIVERA integration for one or more agents at a given location.
...
async def connections_uninstall(body: dict):
    """Remove RIVERA integration for a single agent at a given location.
...
async def shutdown_server(background_tasks: BackgroundTasks):
    """
    Gracefully shutdown the RIVERA server.
    Called by the UI when the browser tab is closed.
    """
    if not settings.RIVERA_UI_MODE:
...
def _migrate_compact_metrics(provider: str, metrics: dict) -> dict:
    ...
    return {
        ...
        "read_ms_rivera": latency.get("rivera_read_ms", 0),
...
def _migrate_load_or_export(
    ...
    """
    from rivera.cli.analyze.letta_export import run_letta_export
    from rivera.cli.analyze.mem0_export import run_mem0_export
    from rivera.cli.analyze.supermemory_export import run_supermemory_export
    from rivera.cli.migrate.runner import load_export
...
def _migrate_get_metrics_fn(provider: str):
    if provider == "mem0":
        from rivera.cli.analyze.mem0_compare import compute_metrics

        return compute_metrics
    if provider == "letta":
        from rivera.cli.analyze.letta_compare import compute_metrics

        return compute_metrics
    from rivera.cli.analyze.supermemory_compare import compute_metrics
...
async def migrate_dry_run(body: dict):
    ...
    """
    from rivera.cli.migrate.runner import map_export, source_count
...
async def migrate_import(body: dict):
    ...
    """
    from rivera.cli.migrate.runner import run_migration
    ...
    client = _build_ui_direct_client()
    if client is None:
        raise HTTPException(status_code=400, detail="No Rivera API key configured.")
