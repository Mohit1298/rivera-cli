...
codebase imports from `rivera/app/legacy/`, and this folder is **excluded from
...
the `rivera conflicts` CLI, not by supersession flags. All of the following were
deleted:
...
**`rivera/app/services/memory_read_service.py`**
...
|---|---|---|
| `rivera/app/utils/idempotency.py` | `legacy/idempotency.py` | `IdempotencyHandler` / `handle_write_idempotency` never imported anywhere |
| `rivera/app/utils/tracing.py` | `legacy/tracing.py` | trace span/decorator helpers never imported |
| `rivera/app/utils/safe_deletion.py` | `legacy/safe_deletion.py` | `SafeDeletion.perform_safe_deletion` never imported |
| `rivera/app/models/phase_d.py` | `legacy/phase_d.py` | Phase-D models never imported |
| `rivera/app/models/universal_endpoints.py` | `legacy/universal_endpoints_models.py` | referenced only by the already-dead legacy `universal_*` files (renamed to avoid colliding with the existing `legacy/universal_endpoints.py` routes file) |
...
`rivera_agent_{agent_id}`. The broader multi-scope abstraction
...
- `MemoryReadService.search_multi_scope()`: removed. Called only from the dead
  `legacy/memory.py`, and it had a broken `from rivera.rivera.app.constants`
  import (double `rivera`) proving it never ran.
...
its namespace is `rivera_agent_{agent_id}` (unchanged on the wire).
...
- `mypy rivera` → no issues (legacy excluded)
- `import rivera.app.main` / `import rivera.cli.main` → OK
