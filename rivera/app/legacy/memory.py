...
from rivera.app.utils.errors import map_error_to_http_exception
...
async def batch_write_memories(
    ...
    Leverages Rivera's batch upload capability for efficient storage.
...
async def update_memory(
    ...
    Since Rivera doesn't support in-place updates, this endpoint:
...
async def search_memories(
    ...
    memory_types: list[str] | None = Query(
        None, description="Memory type filters (leverages Rivera #type:value)"
    ),
    tags: list[str] | None = Query(
        None, description="Tag filters (leverages Rivera #keyword)"
    ),
    ...
    min_similarity_score: float | None = Query(
        ...
        ge=0.0,
        le=1.0,
        description="Minimum similarity score (uses Rivera kiosk_mode)",
    ),
    ...
    limit: int = Query(
        10, ge=1, le=100, description="Result limit (maps to Rivera top_k)"
    ),
    offset: int = Query(
    ...
    """
    Search memories with Rivera's native metadata filtering

    Leverages Rivera's #key:value syntax for server-side filtering
...
async def search_multi_scope(
    ...
    Leverages Rivera's multi-namespace search to find memories
