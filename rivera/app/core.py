"""
RIVERA Core Architecture - Namespace Strategy & Memory Records
"""
...
def agent_namespace(agent_id: str) -> str:
    """Map an agent_id to its Rivera namespace: rivera_agent_{agent_id}."""
    return f"rivera_agent_{agent_id}"
...
class MemoryRecord(BaseModel):
    ...
    def to_moorcheh_document(self) -> dict[str, Any]:
        """
        Convert to Rivera document format with flat metadata fields.

        Rivera stores metadata as flat fields on the document, which enables
        ...
        document = {
            "id": self.id,
            "text": text,
            # Metadata fields (flat structure for Rivera filtering)
    ...
    def namespace(self) -> str:
        """The Rivera namespace this memory belongs to."""
