...
Replaces tenant_id with Rivera API key-based authentication.
"""
...
from rivera.app.clients import moorcheh as moorcheh_clients
from rivera.app.config import settings
from rivera.app.models.session import (
    AgentCreate,
    AgentInfo,
    AgentList,
    Session,
...
from rivera.app.routes import memory  # noqa: E402
from rivera.app.routes.auth_deps import (  # noqa: E402
    get_current_session,
...
async def _namespace_item_counts(moorcheh_api_key: str) -> dict[str, int]:
    """Map namespace_name -> live document count from Rivera.

    The ``memory_count`` stored in local agent metadata is never updated after
    creation, so it is always 0. Rivera tracks the authoritative per-namespace
    document count, which is what the UI should display. Best-effort: returns an
    empty map if Rivera is unreachable so agent listing still succeeds.
    """
    try:
...
async def create_agent(
    ...
    """
    Create a new RIVERA agent

    Creates:
    - Agent metadata in ~/.rivera/agents/
    - Rivera namespace: rivera_agent_{agent_id}
...
async def list_agents(moorcheh_api_key: str = Depends(verify_moorcheh_api_key)):
    """
    List all agents for this Rivera account

    Returns agents sorted by creation date (newest first). The ``memory_count``
    of each agent is populated with the live document count from its Rivera
...
async def get_agent(
    ...
    ``memory_count`` reflects the live document count from the agent's Rivera
    namespace.
    """
...
async def delete_agent(
    ...
    delete_backup_too: bool = Query(
        False, alias="delete-backup-too", description="Delete Rivera namespace backup"
    ),
    ...
    If `delete-backup-too=true`, also deletes the agent memory namespace in Rivera.
    """
    try:
        agent = agent_service.get_agent(agent_id)
        if not agent:
        ...
        return {
            "message": (
                ...
                + (
                    ...
                    if delete_backup_too
                    else " (backup retained in Rivera)"
                )
            )
        }
...
async def activate_agent(
    ...
    - Session file in ~/.rivera/sessions/
