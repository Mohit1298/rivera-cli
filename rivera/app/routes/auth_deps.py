...
def get_moorcheh_api_key() -> str:
    """
    Get Rivera API key from server configuration.
    ...
    if parse_backend(settings.RIVERA_BACKEND) == Backend.ON_PREM:
    ...
    if settings.RIVERA_API_KEY:
        return settings.RIVERA_API_KEY

    raise HTTPException(
        status_code=500,
        detail="Server misconfigured: RIVERA_API_KEY is not set",
    )


def verify_moorcheh_api_key() -> str:
    """
    Return configured Rivera API key.
