...
shape that the cloud SDK does, so Rivera's service layer never branches on
...
``~/.rivera/on-prem/state.json`` via ``config_manager.get_answer_config()``,
...
class AsyncOnPremClient(OnPremClient):
    """Async facade. Rivera only awaits a handful of methods via
