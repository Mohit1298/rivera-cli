"""
Backend selection and protocol for Rivera's Rivera client.

Rivera can talk to two backends:
...
def get_active_llm_model(cloud_default: str) -> str | None:
    ...
    On-prem: returns ``llm_model`` from ``~/.rivera/on-prem/state.json`` (set
    ...
    if parse_backend(settings.RIVERA_BACKEND) == Backend.CLOUD:
        return cloud_default

    state_path = Path.home() / ".rivera" / "on-prem" / "state.json"
