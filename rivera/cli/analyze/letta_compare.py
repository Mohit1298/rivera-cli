"""
Compare a Letta archival memory export against Rivera.
...
def compute_metrics(export: dict[str, Any]) -> dict[str, Any]:
    ...
    storage_saved_bytes = storage_letta_bytes - storage_rivera_bytes

    read_ms_letta = ASSUMPTIONS["letta_read_ms"]
    read_ms_rivera = ASSUMPTIONS["rivera_read_ms"]
    latency_speedup = (
        round(read_ms_letta / read_ms_rivera, 1) if read_ms_rivera else 0
    )
    ...
    return {
        ...
        "latency": {
            ...
            "ms_saved_per_query": read_ms_letta - read_ms_rivera,
        },
    }
...
def build_llm_prompt(metrics: dict[str, Any]) -> str:
    ...
    return (
        ...
        "compares a user's existing Letta deployment against Rivera "
        ...
        f"${t['letta_extraction_cost_usd']} total. With Rivera, typed "
        f"primitives could bypass that step → 0 tokens, $0.\n"
        f"2. Latency — Letta archival recall is ~{lat['letta_read_ms']}ms read "
        f"with embedding search over passages. Rivera could deliver "
        ...
        f"({s['letta_human']} across {v['vector_count']} vectors). Rivera "
        ...
        "Neighbor (ANN) search over archival passages; Rivera would use "
...
def build_report_markdown(
    *,
    ...
    lines: list[str] = []
    lines.append("# Rivera vs. Letta — Memory Analysis Report")
    lines.append("")
    ...
    lines.append("| --- | --- | --- |")
    lines.append(
        f"| Read latency | ~{lat['letta_read_ms']}ms | <{lat['rivera_read_ms']}ms |"
    )
    ...
    lines.append("| | Letta | Rivera |")
    lines.append("| --- | --- | --- |")
    lines.append(f"| Vector storage | {s['letta_human']} | {s['rivera_human']} |")
    lines.append("")
    lines.append(
