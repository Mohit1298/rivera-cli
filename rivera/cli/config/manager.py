"""
RIVERA CLI Configuration Manager

Handles configuration persistence:
  - API key: stored in ~/.rivera/.env (sensitive, not committed)
  - Other config: stored in ~/.rivera/config.yaml (non-sensitive)
  - Connections registry: stored in ~/.rivera/connections.json
"""
...
class ConfigManager:
    """Manages RIVERA CLI configuration.

    API key lives in ``~/.rivera/.env`` (plain-text, owner-only permissions).
    Everything else (server, session, CLI prefs, active session) lives in
    ``~/.rivera/config.yaml``.
    """

    def __init__(self, config_dir: Path | None = None):
        self.config_dir = config_dir or Path.home() / ".rivera"
        ...
        # Load env vars from the rivera .env file
    ...
    def get_api_key(self) -> str | None:
        """Get Rivera API key from ~/.rivera/.env."""
        # Re-read from file each time to pick up changes
        if self.env_file.exists():
            load_dotenv(self.env_file, override=True)
        key = os.environ.get("RIVERA_API_KEY", "").strip()
        return key if key else None

    def set_api_key(self, api_key: str) -> None:
        """Save Rivera API key to ~/.rivera/.env."""
        self._set_env_var("RIVERA_API_KEY", api_key)

    def get_supermemory_api_key(self) -> str | None:
        """Get Supermemory API key from ~/.rivera/.env."""
        if self.env_file.exists():
            load_dotenv(self.env_file, override=True)
        key = (
    ...
    def set_letta_api_key(self, api_key: str) -> None:
        """Save Letta API key to ~/.rivera/.env."""
        self._set_env_var("LETTA_API_KEY", _normalize_duplicated_api_key(api_key))

    def _set_env_var(self, name: str, value: str) -> None:
        """Write a single variable to ~/.rivera/.env and update os.environ."""
        if not self.env_file.exists():
            self.env_file.write_text("# RIVERA Environment\n")
    ...
    # On-prem config — strictly isolated under ~/.rivera/on-prem/state.json.
    ...
    def get_onprem_config(self) -> dict:
        ...
        Sourced exclusively from ``~/.rivera/on-prem/state.json``; defaults
    ...
    def set_onprem_config(
        ...
        """Persist on-prem config values into ``~/.rivera/on-prem/state.json``."""
    ...
    def get_data_dir(self) -> Path:
        ...
        Cloud users keep ``~/.rivera/`` (no migration). On-prem data is
        isolated under ``~/.rivera/on-prem/`` so switching backends does
    ...
    def load_yaml(self) -> dict:
        ...
        try:
            ...
            rivera_data = data.get("rivera", {})
            if not isinstance(rivera_data, dict):
                return {}
    ...
    def save_yaml(self, data: dict) -> None:
        """Save dict to config.yaml under the 'rivera' key."""
        with open(self.config_file, "w") as f:
            yaml.dump({"rivera": data}, f, default_flow_style=False, sort_keys=False)
        try:
    ...
    def get_answer_config(self) -> dict:
        ...
        ``~/.rivera/on-prem/state.json`` (set during onboarding). All other
    ...
    # Active session tracking — sourced from SessionService (~/.rivera/sessions/).
    ...
    # Connections registry — tracks which agents have rivera installed where.
    # Forward-only: only updated by future install/remove calls, not backfilled.

    def load_connections(self) -> dict:
        """Load the connections registry from ~/.rivera/connections.json."""
