...
Pipeline:
    ...
    2. Map source rows → Rivera memory payloads via ``mappers.MAPPERS``.
...
from rivera.cli.migrate.mappers import MAPPERS, type_breakdown
...
def write_preview(rows: list[dict[str, Any]], dest: Path) -> Path:
    """Write the mapped Rivera payloads so a dry-run is fully inspectable."""
