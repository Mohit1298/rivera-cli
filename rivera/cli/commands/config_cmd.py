"""
RIVERA CLI - Config commands (show, backend).
"""
...
def config_show():
    ...
    table = Table(title="RIVERA Configuration", show_header=False)
...
def config_backend(
    ...
    """Show or switch the active Rivera backend.
    ...
    if target not in {"cloud", "on-prem"}:
        _error(
            ...
            hint="Use: rivera config backend cloud  OR  rivera config backend on-prem",
        )
    ...
    if target_backend == Backend.CLOUD:
        if not config_manager.get_api_key():
            from rivera.cli.commands.core import _cloud_setup

            _cloud_setup()
    else:
        from rivera.cli.commands.core import _onprem_setup
    ...
    try:
        ...
        _settings.RIVERA_BACKEND = target_backend.value
