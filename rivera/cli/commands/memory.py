"""
RIVERA CLI - Memory commands (remember, recall, answer, daily-summary,
...
def remember(
    ...
    Single memory:  rivera remember "some fact"
    Batch mode:     rivera remember --batch memories.json
    """
    ...
    if not active_agent_id or not active_session_token:
        _error(
            "No active agent.", hint="Run 'rivera agent activate <agent-id>' first."
        )
    ...
    if not content:
        _error(
            ...
            "Try 'rivera remember --help' for help.",
        )
        ...
        with console.status("[cyan]Searching memories...", spinner="dots"):
            if as_of:
                ...
                _error(
                    ...
                    hint="Try 'rivera recall --help' for help.",
                )
        elapsed = time.perf_counter() - start
...
def daily_summary(
    ...
    if not agent_id:
        if not active_agent_id:
            _error(
                "No active agent.",
                hint="Provide an agent ID or run 'rivera agent activate <agent-id>' first.",
            )
        agent_id = active_agent_id
    ...
    try:
        ...
        console.print(
            ...
            "Run 'rivera detect-conflicts' or enable the schedule "
            "with 'rivera schedule enable'.[/dim]"
        )
...
def detect_conflicts(
    ...
    and writes the JSON report to ``~/.rivera/conflicts/``. Resolve the
    detected conflicts interactively with ``rivera conflicts``.

    This is the command the schedule runs — see ``rivera schedule``.
    """
    start = time.perf_counter()
    active_agent_id, _ = config_manager.get_active_session()
    ...
    try:
        ...
        if conflicts.get("status") == "success":
            ...
            if count > 0:
                ...
                console.print(
                    "[dim]  Run 'rivera conflicts' to resolve interactively[/dim]"
                )
            else:
...
def conflicts(
    ...
    Examples:
        rivera conflicts
        rivera conflicts --date 2026-03-01
        rivera conflicts --list
        rivera conflicts --agent my-agent
    """
    active_agent_id, active_session_token = config_manager.get_active_session()
    ...
    if not unresolved:
        ...
        console.print(
            "[dim]Run 'rivera detect-conflicts' to generate a conflict report.[/dim]"
        )
        return
    ...
    if not active_session_token:
        _error(
            ...
            "Run 'rivera agent activate <agent-id>' first.",
        )
