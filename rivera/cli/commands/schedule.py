"""
RIVERA CLI - Schedule commands (enable, disable, status).
"""
...
from rivera.cli.schedule_manager import ScheduleManager
...
def schedule_run_internal(
    ...
    run ``rivera daily-summary`` or ``rivera detect-conflicts`` instead.
    """
    ...
    if config_manager.get_backend() == Backend.ON_PREM:
        _error(
            ...
            hint="Switch with: rivera config backend cloud",
        )
    ...
    if not agent_id:
        if not active_agent_id:
            _error(
                "No active agent.",
                hint="Run 'rivera agent activate <agent-id>' first.",
            )
    ...
    if result.get("enabled"):
        ...
        console.print(
            Panel(
                ...
                "[dim]Run 'rivera schedule enable' to activate the nightly "
