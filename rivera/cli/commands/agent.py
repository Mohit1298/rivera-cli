"""
RIVERA CLI - Agent commands (create, list, activate, deactivate, delete, bootstrap).
"""
...
def agent_create(
    ...
    try:
        ...
        console.print(
            '[dim]You can now run: rivera remember "..." and rivera recall "..."[/dim]'
        )
...
def agent_list():
    ...
    try:
        ...
        if not agents:
            console.print(
                "[yellow]No agents found. Create one with 'rivera agent create'[/yellow]"
            )
            return

        table = Table(
            title="RIVERA Agents", show_header=True, header_style=BOLD_PRIMARY
        )
...
def agent_activate(
    ...
    try:
        ...
        if "not found" in msg.lower():
            hint = "Run 'rivera agent list' to see available agents, or 'rivera agent create <id>' to create one."
...
def agent_delete(
    ...
    Removes local agent metadata and optionally deletes the Rivera namespace.
    ...
    # Ask whether to keep the Rivera-side memories (namespace) around.
    ...
    if on_prem:
        ...
        console.print(
            "[bold]Keep the agent's namespace on the on-prem Rivera server?[/bold]\n"
    ...
    try:
        ...
        hint = None
        if "not found" in msg.lower():
            hint = "Run 'rivera agent list' to see available agents."
        _error(f"Failed to delete agent '{agent_id}': {msg}", hint=hint)

    if not keep_cloud:
        namespace = f"rivera_agent_{agent_id}"
...
def agent_bootstrap(
    ...
    Examples:
        rivera agent bootstrap
        rivera agent bootstrap my-agent
        rivera agent bootstrap --output snapshot.json
    """
    ...
    if total_sampled == 0:
        ...
        console.print(
            Panel(
                ...
                '[dim]Store memories with: rivera remember "some fact"[/dim]',
