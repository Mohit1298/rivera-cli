"""
RIVERA CLI - Shared utilities, app instances, and helpers.
...
from rivera.app.utils.errors import InvalidSessionTokenError, SessionExpiredError

# Re-export temporal helpers
from rivera.app.utils.temporal_helpers import (  # noqa: F401
...
from rivera.cli.connect.agent_registry import (  # noqa: F401
    AGENT_REGISTRY,
...
from rivera.cli.connect.engine import install_agent, remove_agent  # noqa: F401

# Re-export display functions
from rivera.cli.ui.display import print_logo, show_welcome_banner  # noqa: F401

# Re-export theme constants for all command modules
from rivera.cli.ui.theme import (  # noqa: F401
    ACCENT,
    BOLD_BRIGHT,
    BOLD_PRIMARY,
    BRIGHT,
...
connect_app = typer.Typer(help="Connect RIVERA to external tools")
migrate_app = typer.Typer(
    help="Migrate memories from other providers (Mem0/Letta/Supermemory) into Rivera"
)
...
def get_client() -> SdkClient:
    ...
    if backend == Backend.ON_PREM:
        ...
        if not cfg_key:
            _error(
                "RIVERA not configured.",
                hint="Run 'rivera' to set up your API key.",
            )
        api_key = cfg_key
