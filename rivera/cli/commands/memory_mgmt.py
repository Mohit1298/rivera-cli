"""
RIVERA CLI - Memory management commands (export, sync).
"""
...
from rivera.app.services.memory_export_service import (
    MEMORY_TYPE_META,
    MEMORY_TYPE_ORDER,
)
...
def memory_export(
    ...
    Examples:
        rivera memory export
        rivera memory export --agent my-agent
        rivera memory export -o ./memory.md
        rivera memory export -n 50
    """
    ...
    if not agent_id:
        if not active_agent_id or not active_session_token:
            _error(
                ...
                hint="Provide --agent or run 'rivera agent activate <agent-id>' first.",
            )
...
def memory_sync(
    ...
    Examples:
        rivera memory sync
        rivera memory sync --project-dir ./my-project
        rivera memory sync -p C:\\\\Projects\\\\my-app --agent my-agent
    """
    start = time.perf_counter()
    active_agent_id, active_session_token = config_manager.get_active_session()
