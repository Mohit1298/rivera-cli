...
Outputs live in ``~/.rivera/migrate/<provider>/<timestamp>/`` to keep the
...
from rivera.cli.migrate.runner import (
    load_export,
    run_migration,
    write_preview,
)
...
def _resolve_provider_key(
    ...
    if not entered or not entered.strip():
        _error(
            ...
            hint=f"Pass --api-key or set {env_name} in ~/.rivera/.env",
        )
    set_fn(entered.strip())
    console.print("[green]  ✓ API key saved to ~/.rivera/.env[/green]")
    resolved = get_fn()
    if not resolved:
...
def _generate_narrative(prompt: str, *, provider_label: str) -> tuple[str, str, str]:
    ...
    method = (
        "Rivera 'answer' endpoint over the active agent's namespace; "
    ...
    if not active_agent_id:
        _warn(
            ...
            "Run 'rivera agent activate <agent-id>' to include it."
        )
        ...
        try:
            ...
            result = client.answer(
                ...
                header_prompt=(
                    ...
                    "Rivera benefits. Output clean markdown. Do not fabricate "
                    "benchmark numbers."
                ),
            ...
            last_error = str(exc)
            if attempt == 0:
                _warn("Rivera session invalid — re-activating agent and retrying...")
                try:
...
def _resolve_target_agent(agent: str | None) -> str:
    ...
    if not active_agent_id or not active_session_token:
        _error(
            ...
            hint=(
                "Activate an agent first ('rivera agent activate <id>') "
                "or pass --agent <id>."
            ),
        )
...
def _run_migrate_flow(
    *,
    ...
    console.print(
        Panel.fit(
            f"[{BOLD_PRIMARY}]{label} -> Rivera  {mode}[/{BOLD_PRIMARY}]",
            border_style=PRIMARY,
        )
    )
    ...
    progress("Mapping source records onto Rivera schema...")
...
def migrate_mem0(
    api_key: str | None = typer.Option(
        ...
        "--api-key",
        envvar="MEM0_API_KEY",
        help="Mem0 API key (saved to ~/.rivera/.env)",
    ),
    ...
    agent: str | None = typer.Option(
        ...
        "--agent",
        "-a",
        help="Target Rivera agent id (defaults to the active agent).",
    ),
    ...
    """Migrate a Mem0 account into the active (or selected) Rivera agent.

    Examples:
        rivera migrate mem0 --dry-run
        rivera migrate mem0 --file ./mem0_export.json
        rivera migrate mem0 --agent my-agent --report
    """
    _run_migrate_flow(
        provider="mem0",
        api_key=api_key,
...
def migrate_letta(
    api_key: str | None = typer.Option(
        ...
        help="Letta API key (saved to ~/.rivera/.env)",
    ),
    file: Path | None = typer.Option(
        None,
        "--file",
    ...
    """Migrate Letta archival passages into the active (or selected) Rivera agent."""
    _run_migrate_flow(
        provider="letta",
        api_key=api_key,
        file=file,
...
def migrate_supermemory(
    api_key: str | None = typer.Option(
        ...
        help="Supermemory API key (saved to ~/.rivera/.env)",
    ),
    file: Path | None = typer.Option(
        None,
        "--file",
    ...
    """Migrate a Supermemory account into the active (or selected) Rivera agent."""
    _run_migrate_flow(
