"""
RIVERA Direct Client

Calls the Rivera API directly through existing service classes
"""
...
# Rivera's API Gateway strictly requires lowercase for 'x-api-key'.
    ...
    def _request(self, method: str, endpoint: str, json_data: Any = None) -> Any:
        ...
        try:
            ...
            try:
                ...
                raise Exception(
                    f"Rivera API Error {e.code}: {err_json.get('message', body)}"
                )
            except json.JSONDecodeError:
                raise Exception(f"Rivera API Error {e.code}: {body}")
        except Exception as e:
            raise Exception(f"Rivera Connection Error: {e}")
...
class DirectClient:
    ...
    def __init__(self, api_key: str) -> None:
        ...
        Args:
            api_key: Rivera API key (required, non-empty).
    ...
    def _get_moorcheh(self):
        """Return (or create) the backend-aware Rivera client.
        ...
        if self._moorcheh is None:
            from rivera.app.clients.moorcheh import get_moorcheh_client

            logger.debug("Initializing Rivera client via backend dispatcher")
    ...
    def _get_read_service(self):
        ...
        if self._read_service is None:
            from rivera.app.services.memory_read_service import MemoryReadService
    ...
    def _get_daily_analysis_service(self):
        ...
        if self._daily_analysis_service is None:
            from rivera.app.services.daily_analysis_service import (
    ...
    def _get_export_service(self):
        ...
        if self._export_service is None:
            from rivera.app.services.memory_export_service import MemoryExportService
    ...
    def _get_validated_session_for_agent(self, agent_id: str):
        ...
        # writes the new token to ~/.rivera/sessions/{agent}.json and
    ...
    def create_agent(
        ...
        from rivera.app.models.session import AgentCreate, AgentPattern
    ...
    def answer(
        ...
        to the Rivera LLM to generate a grounded answer.
    ...
    def generate_conflict_report(self, agent_id: str, date: str) -> dict[str, Any]:
        ...
        memories and writes the JSON report to ``~/.rivera/conflicts/``.
    ...
    def list_conflicts(
        ...
        json_path = (
            Path.home() / ".rivera" / "conflicts" / f"{agent_id}_{date}_conflicts.json"
        )
    ...
    def resolve_conflict(
        ...
        from rivera.app.core import agent_namespace
    ...
    def export_memory_md(
        ...
        Args:
            ...
            output_path: Custom output path. Defaults to
                ``~/.rivera/exports/{agent_id}_memory.md``.
        ...
        from rivera.app.services.memory_export_service import MEMORY_TYPE_ORDER
    ...
    def sync_memory_to_project(
        ...
        Uses the cached export at ``~/.rivera/exports/{agent_id}_memory.md``
        ...
        cache_path = Path.home() / ".rivera" / "exports" / f"{agent_id}_memory.md"
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        exists only for interface compatibility with ``RiveraAPIClient``.
        ...
        raise ConnectionError(
            ...
            "Run 'rivera serve' to start the server if needed."
        )
