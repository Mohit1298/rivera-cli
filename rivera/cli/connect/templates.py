"""
RIVERA CLI - Connect Templates

Per-agent instruction content and skill templates for RIVERA integration.
"""


# Shared RIVERA Sentinel markers

RIVERA_SENTINEL = "<!-- RIVERA-MANAGED-SECTION -->"
RIVERA_SENTINEL_END = "<!-- /RIVERA-MANAGED-SECTION -->"
...
SKILL_MD_CONTENT = """---
name: rivera-memory
description: Use this skill when you need to store or search RIVERA persistent memories. It defines mandatory guidelines for best practices, memory types, confidence levels, tagging, and patterns for effective agent memory usage.
---

# RIVERA Memory Skill

Detailed reference for using RIVERA persistent memory effectively.
...
```bash
rivera remember "Implemented X using approach Y because Z. Commit abc123." --type decision --tags "feature-x" --confidence 0.95 --provenance "inferred" --source "claude_code"
rivera remember "Learned that batch ops reduce API calls 100x." --type learning --tags "performance" --confidence 0.85 --provenance "observed" --source "claude_code"
```

### When User Corrects You
```bash
rivera remember "User corrected: prefer pytest over unittest." --type learning --tags "correction,testing" --confidence 1.0 --provenance "corrected" --source "claude_code"
```
...
rivera recall "authentication approach" --limit 10

# Use answer — need a direct synthesized answer
rivera answer "What auth approach did we decide on and why?"
```
...
4. **Duplicates** — Search first (`rivera recall`), then store if not found
...
```bash
# Store memory
rivera remember "content" --type TYPE --tags "tag1,tag2" --confidence 0.9 --provenance "inferred" --source "claude_code"

# Raw memory search (use for context-building, multi-step tasks)
rivera recall "query" --limit 10 --type TYPE --min-similarity 0.8

# Temporal recall variants (no query needed)
rivera recall --recent --limit 10                 # newest first
rivera recall --as-of "2026-01-15"                # state at a point in time
rivera recall --changed-since "last 7 days"       # what changed since

# Synthesized answer (use for direct questions, "what did we decide about X?")
rivera answer "question"

# Sync memories to project
rivera memory sync --project-dir .
```
"""
...
def _base_instruction_content(
    ...
    """Generate base RIVERA instruction content, adapted per agent."""
    note_line = ""
    if note_suffix:
        note_line = f"\n> **Note**: {note_suffix}"

    return f"""{RIVERA_SENTINEL}
## RIVERA - Your Active Memory Companion

**RIVERA is not a passive store. It is an active companion agent that works alongside you.**
Don't treat RIVERA like a static blob you query once and forget. It's a teammate you keep
talking to, every preference, decision, and correction flows through it. RIVERA remembers,
...
Every memory operation in this session goes through RIVERA. There is no exception.

> **CRITICAL**: All `rivera` commands are **shell commands**. Always run them using {tool_phrase}.
...
3. **Store proactively. Do not wait to be asked.** The moment a memory-worthy event happens
   ...
   a mistake corrected — run `rivera remember` immediately, in the same turn.
4. **Always pass full metadata to `remember`.** Every `rivera remember` call MUST include
...
5. **One memory operation goes through RIVERA. All of them do.** Do not keep mental notes,
   in-context scratch pads, or "I'll remember this for next time" promises. If it matters
   beyond this turn, it goes into RIVERA. If it doesn't, drop it.

### Memory Operations — Use the Right One

RIVERA gives you three primitives. They are equal-priority. Pick by intent, not by habit.

| You want to... | Use | Why |
|---|---|---|
| Read raw memory chunks and apply them as context | `rivera recall "query"` | Best for context-building, multi-step work, comparing options |
| Get one synthesized, grounded answer to a direct question | `rivera answer "question"` | Best for "what did we decide / prefer / commit to?" — saves you reading and merging |
| Persist something memory-worthy | `rivera remember "content" --type ... --confidence ... --provenance ... --source ...` | Every preference, decision, fact, instruction, goal, lesson |
| See what changed since last time | `rivera recall --changed-since "last 7 days"` | Catching up after a break |
| See the most recent memories | `rivera recall --recent` | Fast context refresh |
...
- User says *"I prefer tabs over spaces"*:
  `rivera remember "User prefers tabs over spaces for indentation" --type preference --confidence 1.0 --provenance explicit_statement --source <your_agent_name>`
- You decide to use Library X for reason Y:
  `rivera remember "Chose Library X for reason Y; commit abc123" --type decision --confidence 0.95 --provenance inferred --source <your_agent_name>`
- User corrects an approach:
  `rivera remember "User corrected: use pytest, not unittest" --type learning --confidence 1.0 --provenance corrected --source <your_agent_name>`
- A failed approach taught you something:
  `rivera remember "Batch size > 100 fails with TimeoutError" --type error --confidence 0.95 --provenance observed --source <your_agent_name>`

### Command Reference

```bash
# Store — ALWAYS pass full metadata
rivera remember "content" --type <type> --confidence <0.0-1.0> --provenance <provenance> --source <agent_name>

# Recall raw context
rivera recall "query"                              # semantic search
rivera recall "query" --type <type> --limit 10     # filtered search
rivera recall --recent --limit 10                  # newest first, no query
rivera recall --as-of "2026-01-15"                 # state at a point in time
rivera recall --changed-since "last 7 days"        # what changed since
...
def get_instruction_content(agent_name: str) -> str:
    """Get RIVERA instruction section content for a specific agent."""
    templates = {
        "claude-code": _base_instruction_content(
            tool_phrase="the Bash tool",
            note_suffix="The `rivera-memory` skill contains reference guidelines only (best practices, confidence levels, tagging). It is NOT executable — always use Bash for rivera commands.",
        ),
        "codex": _base_instruction_content(
            tool_phrase="the terminal",
            note_suffix="The `rivera-memory` skill in `.agents/skills/rivera/` contains detailed reference guidelines (best practices, confidence levels, tagging).",
        ),
        "cursor": _get_mdc_content(),
        "windsurf": _base_instruction_content(
            tool_phrase="the terminal",
            note_suffix="The `rivera-memory` skill in `.windsurf/skills/rivera/` contains detailed reference guidelines.",
        ),
        "gemini-cli": _base_instruction_content(
            tool_phrase="the terminal",
            note_suffix="The `rivera-memory` skill in `.gemini/skills/rivera/` contains detailed reference guidelines.",
        ),
        "cline": _base_instruction_content(
            tool_phrase="the terminal",
            note_suffix="Run `rivera memory sync --project-dir .` at the start of each session to populate MEMORY.md.",
        ),
        ...
        "opencode": _base_instruction_content(
            tool_phrase="the terminal",
            note_suffix="The `rivera-memory` skill in `.agents/skills/rivera/` contains detailed reference guidelines.",
        ),
        ...
        "augment": _base_instruction_content(
            tool_phrase="the terminal",
            note_suffix="The `rivera-memory` skill in `.augment/skills/rivera/` contains detailed reference guidelines.",
        ),
    }
...
def _get_mdc_content() -> str:
    ...
    return f"""---
description: RIVERA — active memory companion. Mandatory rules for storing, recalling, and answering from persistent memory.
alwaysApply: true
---
...
> **CRITICAL**: All `rivera` commands are **shell commands**. Always run them in the terminal.
> Never simulate or "pretend to call" them. If you cannot run the shell, say so explicitly
> instead of inventing memory state.

## NON-NEGOTIABLE RULES

1. **Sync and read `MEMORY.md` at session start.** Run `rivera memory sync --project-dir .`
...
3. **Store proactively. Do not wait to be asked.** The moment a memory-worthy event
   ...
   a goal set, a mistake corrected — run `rivera remember` immediately, in the same turn.
4. **Always pass full metadata to `remember`.** Every call MUST include `--type`,
   ...
   default.
5. **All memory operations go through RIVERA.** No mental scratch pads, no "I'll
   remember this next time." If it matters beyond this turn, it goes into RIVERA.

## Memory Operations — Use the Right One

```bash
rivera recall "query"      # raw memory chunks — for context-building, multi-step work
rivera answer "question"   # one synthesized, grounded answer — for direct questions
rivera recall --recent     # newest first, no query needed
rivera recall --changed-since "last 7 days"
rivera remember "content" --type <type> --confidence <0.0-1.0> --provenance <provenance> --source <agent_name>
rivera memory sync --project-dir .
```
