...
class AgentDef:
    ...
    sentinel: str = "<!-- RIVERA-MANAGED-SECTION -->"
    sentinel_end: str = "<!-- /RIVERA-MANAGED-SECTION -->"
    ...
    def resolve_skill_local(self, project_dir: Path) -> Path:
        ...
        if self.skill_local_dir:
            return project_dir / self.skill_local_dir / "rivera"
        return project_dir / ".agents" / "skills" / "rivera"

    def resolve_skill_global(self) -> Path:
        """Resolve global skill directory path."""
        if self.skill_global_dir:
            return Path.home() / self.skill_global_dir.lstrip("~/") / "rivera"
        return Path.home() / ".agents" / "skills" / "rivera"
...
CLAUDE_CODE = AgentDef(
    ...
    hook_config=AgentHookConfig(
        ...
        hook_payload={
            ...
            "hooks": [
                {
                    "type": "command",
                    "command": "rivera memory sync --project-dir .",
                    "timeout": 30,
                }
            ],
        },
    ...
    permissions_payload={"permissions": {"allow": ["Bash(rivera:*)"]}},
)
...
CURSOR = AgentDef(
    name="cursor",
    display_name="Cursor",
    instruction_file=".cursor/rules/rivera.mdc",
...
CLINE = AgentDef(
    name="cline",
    display_name="Cline",
    instruction_file=".clinerules/rivera.md",
    instruction_format="markdown",
...
CONTINUE = AgentDef(
    ...
    instruction_file=".continue/rules/rivera.md",
    instruction_format="markdown",
    instruction_is_dir=True,
...
ROO = AgentDef(
    ...
    instruction_file=".roo/rules/rivera.md",
    instruction_format="markdown",
    instruction_is_dir=True,
...
AUGMENT = AgentDef(
    ...
    instruction_file=".augment/rules/rivera.md",
    instruction_format="markdown",
    instruction_is_dir=True,
...
def detect_rivera_installed(project_dir: Path) -> list[AgentDef]:
    """Detect which agents have RIVERA already installed (local)."""
    installed = []
    for agent in AGENT_REGISTRY.values():
...
def detect_rivera_installed_global() -> list[AgentDef]:
    """Detect which agents have RIVERA installed globally."""
    installed = []
    for agent in AGENT_REGISTRY.values():
