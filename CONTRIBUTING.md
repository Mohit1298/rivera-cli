...
```bash
cp .env.example .env
# Edit .env and add your RIVERA_API_KEY
```
...
Tests require a valid `RIVERA_API_KEY` in your environment or `.env` file for any integration tests. Unit tests that don't hit the network will run without a key.
